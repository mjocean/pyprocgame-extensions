...
class HD_InitialEntryMode(Mode):
	...
	*left_text* and *right_text* are strings or arrays to be displayed at the
	...
	def __init__(self, game, priority, left_text, right_text, entered_handler):
		...
		# Draw my fancy rows
		self.char_optsF = dmd.Frame(width=self.space_per_char*(self.columns_of_chars_in_palette+1), height=self.space_per_char*(30/self.columns_of_chars_in_palette+1.5))
...
class HD_InitialEntryMode_old(Mode):
	...
	*left_text* and *right_text* are strings or arrays to be displayed at the
	...
	def animate_to_index(self, new_index, inc = 0):
		...
		#print rng
		for x in rng:
			frame = dmd.Frame(width=480, height=self.init_font_height+2)
			for offset in range(-7, 8):
				...
				self.letters_font.draw(frame, self.letters[index], 480/2 - offset * letter_spread - letter_width/2 + x, 0)
